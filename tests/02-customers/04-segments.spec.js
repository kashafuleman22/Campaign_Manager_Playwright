import { test, expect } from '@playwright/test';
  ...
  const dashboardURL = 'https://ucm.demo-octavebytes.com/unified-campaign-manager/main/customerSegmnent';
  ...
  test('Segments(filters) >> Searching by name', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'Search by Name' }).fill('Ks');
  });
  ...
  test('Segments(filters) >> Create Segments', async ({ page }) => {
    await page.goto(dashboardURL);
    await expect(page).toHaveURL(dashboardURL);
    await page.waitForLoadState('networkidle');
    await page.getByRole('button', { name: 'Create Segment' }).click();
